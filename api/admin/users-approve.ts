...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const rows = await sql`SELECT id, email, name, role FROM users WHERE id = ${Number(id)}`
    const user = rows[0]
    ...
    await sql`UPDATE users SET approved = 1, updated_at = CURRENT_TIMESTAMP WHERE id = ${Number(id)}`
