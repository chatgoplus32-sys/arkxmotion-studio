...
async function handleGetWorkflowInfo(apiKey: string, workflowId: string, res: VercelResponse) {
...
async function fetchWorkflowNodes(apiKey: string, workflowId: string): Promise<any[]> {
  // RunningHub: POST /api/openapi/getJsonApiFormat { apiKey, workflowId } -> { code, data: { prompt: "<json string>" } }
  const endpoints = [
...
function buildUltraHDNodeList(nodes: any[], opts: {
  ...
  for (const n of nodes) {
    ...
    // Prompt / negative prompt on CLIP text nodes
  ...
  // Second pass for fps if missed (generic int node with fps-ish title)
...
// Cari parameter tambahan (fps/steps/batas frame/prompt) dari workflow JSON.
// Dipakai melengkapi node image/video resmi (30/33) kalau auto-map penuh gagal.
function collectUltraHDExtras(nodes: any[], opts: {
  ...
  for (const n of nodes) {
    ...
    const title = String(n.title || '')
    const tag = `${cls} ${title}`
    // Lewati loader image/video — sudah dipegang node 30/33
...
async function handleMotionControlUltraHD(apiKey: string, params: any, res: VercelResponse) {
  const {
    workflow_id,
    workflowId,
    imageBase64,
  ...
  // Node image/video resmi workflow Ultra HD: 30 = image, 33 = video.
  // Parameter fps/steps/frames/prompt ditambah kalau discovery menemukannya.
...
async function handleSubmitTryOn(apiKey: string, params: any, res: VercelResponse) {
  const {
    workflow_id,
    workflowId,
    mode = 'tryon',
  ...
  // Prompt tambahan kalau workflow menyediakannya (best-effort via discovery).
  // Hanya field teks — fps/steps/frames tidak relevan untuk try-on.
  try {
    const nodes = await fetchWorkflowNodes(apiKey, effectiveWorkflowId)
    const extras = collectUltraHDExtras(nodes, {
...
async function handleSubmitAudioAvatar(apiKey: string, params: any, res: VercelResponse) {
  const {
    workflow_id,
    workflowId,
    imageBase64,
...
async function handleSubmitVideoUpscale(apiKey: string, params: any, res: VercelResponse) {
  const {
    workflow_id,
    workflowId,
    videoBase64,
  ...
  // Dok workflow ini generik — nama field video aslinya dicari otomatis.
  // 803 field_not_found = validasi gratis (task tidak jalan, koin tidak kepakai).
      ...
      if (mm && /field_not_found|node_not_found/i.test(mm.reason)) {
        ...
        if (isVideoEntry) {
          if (/field_not_found/i.test(mm.reason)) {
            ...
            break // kandidat video berikutnya
          }
          return res.status(200).json({ ok: false, error: `Node video (1) tidak ada di workflow ini`, data: r.data })
        }
        // Entri non-video bermasalah → buang (pakai default workflow), submit ulang
...
async function handleSubmitPhotoEnhance(apiKey: string, params: any, res: VercelResponse) {
  const {
    workflow_id,
    workflowId,
    imageBase64,
  ...
  // Dok bisa generik — nama field image dicari otomatis, entri asing dibuang.
...
async function handleSubmitImageEdit(apiKey: string, params: any, res: VercelResponse) {
  const {
    workflow_id,
    workflowId,
    imageBase64,
  ...
  const effectiveWorkflowId = workflow_id || workflowId || RUNNINGHUB_IMAGE_EDIT_WORKFLOW_ID

  console.log(`[runninghub] Uploading image (image-edit)...`)
  const imageUpload = await rhUpload(apiKey, imageBase64, imageFileName, imageMimeType)
  console.log(`[runninghub] Image uploaded: ${imageUpload.fileName}`)
  ...
  const postRun = async (list: any[]) => {
    ...
    const rawText = await apiRes.text()
    console.log(`[runninghub] submit-image-edit ${apiRes.status}:`, rawText.slice(0, 1000))
    let data: any
    try { data = JSON.parse(rawText) } catch { data = { raw: rawText } }
    return {
      http: apiRes.status,
  ...
  const buildList = () => {
    const list: any[] = [
      { nodeId: '28', fieldName: nodeField['28'], fieldValue: imageUpload.fileName },
  ...
  for (let round = 0; round < 14; round++) {
    ...
    let r: Awaited<ReturnType<typeof postRun>> | null = null
    for (let attempt = 1; attempt <= 3; attempt++) {
      console.log(`[runninghub] image-edit round=${round} (attempt ${attempt}/3)`)
      r = await postRun(list)
      if (r.taskId || (r.http !== 429 && r.code !== 429 && r.code !== 421 && r.code !== '421')) break
      if (r.http === 429 || r.code === 429) {
        return res.status(200).json({ ok: false, error: 'Rate limit exceeded', data: r.data, retryable: true })
    ...
    if (mm && /field_not_found|node_not_found/i.test(mm.reason) && ['28', '31'].includes(mm.nodeId)) {
      ...
      nodeField[mm.nodeId] = cands[nextIdx]
      console.log(`[runninghub] image-edit node ${mm.nodeId}: "${cur}" ditolak → coba "${cands[nextIdx]}"`)
      continue
    }
    if (mm && /field_not_found|node_not_found/i.test(mm.reason)) {
      console.log(`[runninghub] image-edit buang field ${mm.nodeId}/${mm.fieldName} (${mm.reason}), pakai default workflow`)
      droppedEntries.add(`${mm.nodeId}/${mm.fieldName}`)
      continue
    }
    const errorMsg = translateRhError(String(r.code ?? ''), r.msg) || r.msg || 'Submit gagal'
...
async function handleQuery(apiKey: string, taskId: string, res: VercelResponse) {
  ...
  if (mappedStatus === 'COMPLETED') {
    ...
    if (Array.isArray(results) && results.length > 0) {
      // Pass 1: file video; pass 2: output non-teks apa pun (mis. gif/preview)
