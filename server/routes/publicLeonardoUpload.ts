...
router.all('/', (req: Request, res: Response) => {
  ...
  ;(async () => {
    try {
      ...
      if (fields) {
        if (typeof fields === 'string') { try { parsedFields = JSON.parse(fields) } catch {} }
        else if (typeof fields === 'object') parsedFields = fields as Record<string, string>
      }

      if (parsedFields) {
        const fd = new FormData()
        for (const [k, v] of Object.entries(parsedFields)) fd.append(k, String(v))
        fd.append('file', blob, `image.${extension}`)
        try {
          const postRes = await fetch(presignedUrl, { method: 'POST', body: fd, signal: AbortSignal.timeout(60000) })
          uploadOk = postRes.ok
        } catch {}
      }

      if (!uploadOk) {
        const putRes = await fetch(presignedUrl, { method: 'PUT', headers: { 'Content-Type': mimeType }, body: blob, signal: AbortSignal.timeout(60000) })
        uploadOk = putRes.ok
      }

      if (!uploadOk) return res.status(200).json({ ok: false, error: 'S3 upload failed both methods' })
