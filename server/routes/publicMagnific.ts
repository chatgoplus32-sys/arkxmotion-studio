...
router.post('/', (req: Request, res: Response) => {
  ...
  ;(async () => {
    try {
      ...
      if (action === 'submit') {
        if (!modelKey || !payload) return res.status(400).json({ ok: false, error: 'Missing modelKey or payload' })
        const endpoint = modelKey === 'mag:image-upscaler-precision-v2' ? '/v1/ai/image-upscaler-precision-v2' : '/v1/ai/image-upscaler'
      ...
      if (action === 'status') {
        if (!modelKey || !taskId) return res.status(400).json({ ok: false, error: 'Missing modelKey or taskId' })
        const endpoint = modelKey === 'mag:image-upscaler-precision-v2' ? `/v1/ai/image-upscaler-precision-v2/${taskId}` : `/v1/ai/image-upscaler/${taskId}`
        const apiRes = await fetch(`${MAGNIFIC_API}${endpoint}`, {
          method: 'GET',
