...
router.all('/', (req: Request, res: Response) => {
  ...
  ;(async () => {
    try {
      ...
      if (!data) return res.status(200).json({ ok: false, error: 'Failed to parse response', raw: rawText.slice(0, 300) })
      if (data.error_code === 98) return res.status(200).json({ ok: false, error_code: 98, error: data.error_msg || 'Token rejected', data })
      const hasError = data.error_code && data.error_code !== 0
      return res.status(200).json({ ok: !hasError && gatewayRes.ok, data })
    } catch (err: any) {
