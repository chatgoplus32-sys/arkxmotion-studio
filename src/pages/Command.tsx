...
const workflows: WorkflowRoute[] = [
  { id: 'motion', label: 'Motion Control', route: '/generate/motion', icon: <Video className="h-5 w-5" />, keywords: ['motion','dance','gerakan','karakter','transfer','animasi','character','move','kling','wan'], description: 'Transfer gerakan video ke karakter', accent: 'from-blue-500 to-yellow-400', hotkey: 'M' },
...
export default function CommandPage() {
  ...
  return (
    <PageContent className="space-y-6">
      <div className="relative overflow-hidden rounded-[24px] border border-white/[0.06] bg-gradient-to-br from-[#00a8ff]/10 via-card to-card p-6">
        ...
        <div className="relative flex items-start justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-2 text-[11px] font-mono tracking-[0.18em] uppercase text-blue-300/80"><CommandIcon className="h-3.5 w-3.5" /> Command Center</div>
        ...
        <CardContent className="p-5 space-y-4">
          <div className="relative">
            ...
            <textarea
              ...
              disabled={loading}
              className="w-full pl-10 pr-4 py-3 bg-white/[0.04] border border-white/10 rounded-2xl text-sm placeholder:text-white/30 focus:outline-none focus:border-[#00a8ff]/40 focus:bg-white/[0.06] transition resize-none"
            />
            <div className="absolute bottom-2 right-2 flex items-center gap-2">
              ...
              <button onClick={handleRun} disabled={!prompt.trim()||loading} className="px-4 py-2 rounded-xl bg-gradient-to-r from-[#00a8ff] to-[#00d4ff] text-black text-sm font-semibold disabled:opacity-40 hover:shadow-[0_6px_16px_rgba(0,168,255,0.3)] transition flex items-center gap-1.5">
            ...
            <div className="flex items-center gap-2 text-xs">
              {STEPS.map((s,i)=> (
                <div key={s} className="flex items-center gap-1.5">
                  <span className={`h-2 w-2 rounded-full ${i<=step?'bg-[#00a8ff] shadow-[0_0_8px_rgba(0,168,255,0.6)]':'bg-white/10'}`} />
          ...
          {history.length>0 && (
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {history.map(h=> (
                <button key={h} onClick={()=>setPrompt(h)} className="px-3 py-1.5 rounded-full bg-white/[0.04] border border-white/10 text-xs hover:bg-white/[0.08] hover:border-[#00a8ff]/30 transition truncate max-w-[220px]">{h}</button>
              ))}
      ...
      <div className="flex items-center gap-2">
        <div className="relative flex-1 max-w-sm">
          ...
          <input value={filter} onChange={e=>setFilter(e.target.value)} placeholder="Filter workflows..." className="w-full pl-9 pr-3 py-2 rounded-xl bg-white/[0.04] border border-white/10 text-sm placeholder:text-white/30 focus:outline-none focus:border-[#00a8ff]/30" />
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {filteredWorkflows.map(wf=> (
          <button key={wf.id} onClick={()=>navigate(wf.route)} className="text-left p-4 rounded-2xl border border-white/5 bg-white/[0.02] hover:bg-white/[0.04] hover:border-[#00a8ff]/20 hover:-translate-y-0.5 hover:shadow-[0_8px_24px_rgba(0,0,0,0.25)] transition group">
            ...
            <div className="mt-3 inline-flex items-center gap-1 text-xs font-medium text-white/30 group-hover:text-[#00a8ff] transition">Buka <ArrowRight className="h-3 w-3 group-hover:translate-x-0.5 transition" /></div>
          </button>
        ))}
      </div>
      ...
      <Card variant="bordered">
        <CardContent className="p-4">
          <div className="text-sm font-semibold flex items-center gap-2"><Star className="h-4 w-4 text-blue-400" /> Quick Templates</div>
          <div className="mt-3 flex flex-wrap gap-2">
            {EXAMPLES.map(ex=> (
              <button key={ex} onClick={()=>{setPrompt(ex); textareaRef.current?.focus()}} className="px-3 py-1.5 rounded-full bg-white/[0.04] border border-white/10 text-xs hover:bg-[#00a8ff]/10 hover:border-[#00a8ff]/30 hover:text-blue-200 transition">{ex}</button>
            ))}
          </div>
        </CardContent>
      </Card>
