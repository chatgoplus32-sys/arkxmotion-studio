...
import { Loader2, Upload, Download, X, Image as ImageIcon, Sparkles, CheckCircle2, AlertCircle, Clock } from 'lucide-react'
import { useToastStore } from '@/stores/toastStore'
import { submitRunningHubH3I2V, pollRunningHubTask } from '@/lib/runninghub'
...
export default function H3ImageToVideoPage() {
  const addToast = useToastStore((s) => s.addToast)

  const [photoFile, setPhotoFile] = useState<File | null>(null)
  const [photoPreview, setPhotoPreview] = useState<string | null>(null)
  const [prompt, setPrompt] = useState('')
  const [steps, setSteps] = useLocalStorage('h3i2v.steps', 8)
  const [aspect, setAspect] = useLocalStorage('h3i2v.aspect', 'original')
  ...
  const finishSuccess = (url: string, tid: string) => {
    ...
    addToast('✅ H3 Image-to-Video selesai!', 'success')
  }
  ...
  const runWorkflow = async () => {
    ...
    if (!prompt.trim()) {
      addToast('⚠️ Prompt wajib diisi untuk H3 I2V!', 'warning')
      return
    }
    ...
    addLog(`Mulai (H3 I2V): foto ${(upPhoto.size / 1024).toFixed(0)}KB + steps=${steps} aspect=${aspect}`)
    ...
    try {
      ...
      finishSuccess(rotation.result, submittedTaskId || `h3i2v-${Date.now()}`)
    } catch (e: any) {
      setTaskStatus('error')
  ...
  return (
    <PageContent>
      <PageHeader
        title="⚡ H3 Image-to-Video"
        desc="Ubah 1 foto jadi video AI realistis via MiniMax H3 (Ultra-HD Fast 8-Step + FlashVSR). Tulis prompt gerakan + durasi."
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Section title="📸 Input" className="!p-6">
          <div className="mb-4">
            <Label className="mb-2 block">🖼️ Foto Referensi</Label>
            {photoPreview ? (
          ...
          <div className="mb-4">
            <Label className="mb-2 block">✏️ Prompt Gerakan (wajib)</Label>
            <Textarea
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="cth: shot1: A woman moves her hips rhythmically. Close-up camera movement begins from the lower legs... Video duration: [15] seconds"
              rows={4}
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div>
              ...
              <input
                ...
                value={steps}
                onChange={(e) => setSteps(Math.max(1, Math.min(50, Number(e.target.value) || 8)))}
          ...
          <Button
            onClick={runWorkflow}
            disabled={loading || !apiKey || !photoFile || !prompt.trim()}
            className="w-full"
            size="lg"
          >
          ...
          <p className="text-xs text-muted-foreground mt-2">
            Default: 8 Steps · aspect original. Prompt wajib — deskripsikan gerakan kamera + durasi video.
          </p>
        </Section>
