...
export default function UGCPage() {
  ...
  const buildPrompt = (productName: string, idx: number) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!controller.signal.aborted) {
        setStatus({ pct: 100, text: '✅ Selesai' })
  ...
  return (
    <PageContent>
      <PageHeader
        ...
        title="Product UGC"
        highlight="Generator"
        desc="Upload produk → generate foto model menggunakan produk tersebut."
      />

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        {/* Product Upload */}
        <Section title="📦 Foto Produk" sub="Upload foto produk (max 10)">
            ...
            <div className="grid grid-cols-3 gap-2">
              {products.map((p) => (
                <div key={p.id} className="relative aspect-square rounded-lg overflow-hidden border border-border group">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 bg-black/60 text-white text-[9px] px-1 truncate">{p.name}</div>
                </div>
              ))}
              {products.length < 10 && (
                <button
                  onClick={() => productRef.current?.click()}
                  className="aspect-square rounded-lg border border-dashed border-border/80 grid place-items-center hover:border-primary/60 transition"
                >
        ...
        <div className="lg:col-span-2 space-y-5">
          <Section title="⚙️ Pengaturan">
            ...
            <div className="mt-4">
              ...
              <Textarea rows={2} value={extraPrompt} onChange={(e) => setExtraPrompt(e.target.value)} placeholder="Deskripsi tambahan untuk hasil yang lebih spesifik..." />
            </div>
